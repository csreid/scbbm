...
class PlayByPlayDataset(Dataset):
	def __init__(self, df_path, min_len, max_len, size):
		...
		self.teams_id_map = dict(
			[(val, idx) for idx, val in enumerate(self.teams)]
		)
		...
		self.play_ids_map = dict(
			[(val, idx) for idx, val in enumerate(self.plays)]
		)
	...
	def _X_Y(self, subgame):
		X_times = torch.tensor(
			[r.time_elapsed for _, r in subgame[:-1].iterrows()]
		)
		X = torch.tensor(
			[
				self.play_ids_map[r.description]
				for _, r in subgame[:-1].iterrows()
			]
		)
		Y = torch.tensor(
			[
	...
	def _subgame_to_tensor(self, subgame):
		X = torch.tensor(
			[self.play_ids_map[r.description] for _, r in subgame.iterrows()]
		)
