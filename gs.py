...
def data_collate(samples):
	...
	return (
		...
		plays_tens.long(),
		targets_tens,
		times_tens.unsqueeze(2),
		time_targets_tens.unsqueeze(2),
	)
...
class GameSimulator(Module):
	def __init__(self, n_teams, n_plays):
		...
		self.play_input = Embedding(
			n_plays, play_feats, max_norm=1.0, padding_idx=0
		)
		...
		self.recurrent1 = LSTM(
			2 * team_feats + play_feats + 1, 500, num_layers=2
		)
	...
	def forward(self, teams, plays, times):
		...
		with torch.no_grad():
			flat_teams = torch.flatten(
				self.team_input(teams), start_dim=1, end_dim=2
			)
		...
		team_filter_input = team_filter_input.expand(
			1, len(teams), self._n_teams
		).to(dev)
	...
	def scores(self, teams, length=200):
		...
		time_out = F.sigmoid(self.score_time_out(out)) * 3600.0
	...
	def simulate(self, home, away, dataset, maxlen=10):
		...
		while not condition(idx):
			subgame = pd.DataFrame(
				{"description": plays, "time_elapsed": times}
			)
			plays_tens, times_tens = dataset._subgame_to_tensor(subgame)

			with torch.no_grad():
				play_out, time_out = self.forward(
					teams_tens.unsqueeze(0),
					plays_tens.unsqueeze(1),
					times_tens.reshape((-1, 1, 1)),
				)
			...
			next_play_idx = (
				np.random.choice(self._n_plays - 1, p=next_play_dist) + 1
			)
...
def main_sim(gs):
	pbp = PlayByPlayDataset(
		"cleaned_data.csv", min_len=2, max_len=20, size=100000
	)

	batch_size = 32
	loader = DataLoader(
		pbp, batch_size=batch_size, collate_fn=data_collate, shuffle=True
	)
	...
	for epoch in epoch_prog:
		...
		for idx, (team_X, play_X, play_Y, time_X, time_Y) in prog:
			...
			Y_pred_play = (
				Y_pred_play.to("cuda:0").transpose(0, 2).transpose(0, 1)
			)
...
def main_score(gs):
	...
	loader = DataLoader(
		sd, batch_size=batch_size, collate_fn=collate_scores, shuffle=True
	)
	...
	for epoch in epoch_prog:
		prog = tqdm(
			loader, total=int(len(sd) / batch_size), position=1, leave=False
		)
		for X, Y in prog:
			X = X.to("cuda:0")
			Y = Y.to("cuda:0")
			...
			loss_scores = loss_fn(
				Y_pred_scores, Y[:, :, :2]
			)  # + torch.sum((Y_pred_scores - torch.abs(Y_pred_scores)) ** 2)
			...
			if (ctr % 100) == 0:
				...
				with torch.no_grad():
					...
					fig = plt.figure()
					purdue_vals = gs.team_input(
						torch.tensor([sd.teams_id_map["Purdue"]])
					)
					plt.bar(
						np.arange(10), purdue_vals.detach().numpy().squeeze()
					)
					plt.ylim(-1, 1)
					writer.add_figure("Purdue Traits", fig, ctr)

					embs = gs.team_input(
						torch.tensor([sd.teams_id_map[t] for t in sd.teams])
					)
...
if __name__ == "__main__":
	pbp = PlayByPlayDataset(
		"cleaned_data.csv", min_len=2, max_len=5, size=1000000
	)
