...
def fit_on_game(model, teams, plays, subgame_len=20, device='cuda:0'):
	...
	for play_idx in range(0, len(plays)-1, subgame_len):
		...
		Y_plays = plays[play_idx+1:play_idx+subgame_len+1].to('cuda:0', non_blocking=True)
...
if __name__ == '__main__':
	...
	batch_size=32
	loader = DataLoader(ds, batch_size=batch_size, collate_fn=_collate, shuffle=True)
	...
	for epoch in tqdm(range(100)):
		for idx, (teams, game) in tqdm(enumerate(loader), total=int(len(ds) / batch_size), leave=False):
